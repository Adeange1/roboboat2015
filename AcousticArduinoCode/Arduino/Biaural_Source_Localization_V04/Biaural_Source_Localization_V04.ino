// This is a modified version of the .ino file "Binaural_Source_Localization_Benson"
// Modified by Samantha Tropeano on 6/30/15
// This program reads data from two hydrophones and then performs a biaural source localization algorithm that calculates the angle between SeaCat and a sound source (pinger). This angle is then sent to SeaCat.    
// Begin by assigning a value to the "bline" variable.
...
double bline = 1.89; //Hydrophone baseline in inches (25kHz)
...
void loop() {
  ...
  int rfirst = analogRead(A2);  //initial hydrophone samples
  ...
  if ((lfirst <= llowthresh || lfirst >= lhighthresh) && (rfirst <= rlowthresh || rfirst >= rhighthresh))
  {
    ...
    double soundspeed = 62992.12598;  //Speed of sound in water, in/s
    int ITDmax = (int) (bline*fs/soundspeed); //Max interaural time difference, in samples
    ...
    double alpha = asin(soundspeed*I/(bline*fs))*180/pi; //compute the azimuth in degrees corresponding to ITD 'I'
