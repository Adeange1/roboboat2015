 ...
 *
  ...
  *  ------------------------------------------------------------------------- 
 * Created: Tue Jul 22 20:12:11 2014
 * 
 *
 */

#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME lcm_logger_saveV2
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
/* %%%-SFUNWIZ_defines_Changes_BEGIN --- EDIT HERE TO _END */
#define NUM_INPUTS          7
/* Input Port  0 */
#define IN_PORT_0_NAME      pose_data
...
/* Input Port  1 */
#define IN_PORT_1_NAME      vel_data
...
/* Input Port  2 */
#define IN_PORT_2_NAME      euler_data
...
/* Input Port  3 */
#define IN_PORT_3_NAME      portstate_data
...
/* Input Port  4 */
#define IN_PORT_4_NAME      stbdstate_data
...
/* Input Port  5 */
#define IN_PORT_5_NAME      starttime_data
...
/* Input Port  6 */
#define IN_PORT_6_NAME      elapsedtime_data
...
extern void lcm_logger_saveV2_Outputs_wrapper(const real_T *pose_data,
                          const real_T *vel_data,
                          const real_T *euler_data,
                          const real_T *portstate_data,
                          const real_T *stbdstate_data,
                          const real_T *starttime_data,
                          const real_T *elapsedtime_data);
...
char namestr[200]; 
    sprintf(namestr,"lcmlog_%s",time_string); 
     lcmfile = fopen(namestr,"w"); 
    if(lcmfile == NULL){
        printf("LCM File Not Opened\n"); 
        exit(1); 
    }
  }
#endif /*  MDL_START */
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    const real_T   *pose_data  = (const real_T*) ssGetInputPortSignal(S,0);
    const real_T   *vel_data  = (const real_T*) ssGetInputPortSignal(S,1);
    const real_T   *euler_data  = (const real_T*) ssGetInputPortSignal(S,2);
    const real_T   *portstate_data  = (const real_T*) ssGetInputPortSignal(S,3);
    const real_T   *stbdstate_data  = (const real_T*) ssGetInputPortSignal(S,4);
    const real_T   *starttime_data  = (const real_T*) ssGetInputPortSignal(S,5);
    const real_T   *elapsedtime_data  = (const real_T*) ssGetInputPortSignal(S,6);

        fprintf(lcmfile,"Pose: %g %g %g\n",pose_data[0],pose_data[1],pose_data[2]); 
    fprintf(lcmfile,"Vel: %g %g\n",vel_data[0],vel_data[1]); 
    fprintf(lcmfile,"Euler: %g %g %g\n",euler_data[0],euler_data[1],euler_data[2]); 
    fprintf(lcmfile,"Port: %g %g\n",portstate_data[0],portstate_data[1]); 
    fprintf(lcmfile,"Stbd: %g %g\n",stbdstate_data[0],stbdstate_data[1]); 
    fprintf(lcmfile,"StartTime: %g\n",*starttime_data); 
    fprintf(lcmfile,"ElapsedTime: %g\n\n",*elapsedtime_data); 
