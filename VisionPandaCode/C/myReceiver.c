 ...
 *
  ...
  *  ------------------------------------------------------------------------- 
 * Created: Mon Jun 23 20:43:19 2014
 * 
 *
 */

#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME myReceiver
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
extern void myReceiver_Outputs_wrapper(int32_T *task);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    if(read(fd,readBuffer,sizeof(readBuffer))>0 && 
       ...
       *task = atoi(placeHolder); printf("ReadBuffer:%s\n",readBuffer); 
    }
       
    //myReceiver_Outputs_wrapper(task);
}
