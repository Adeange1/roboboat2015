/*
 * File: lidarCam_Select_June02_15.c
 * Author: Gin Cheng, Wesley Anderson
 * Villanova University
 * Date: 6/02/2015
 * Zero the gimbal then do nothing
 * Remaining code is commented out to allow for easy implementation
 * once zeroing is fixed
 *
...
#define S_FUNCTION_NAME lidarCam_Select_June02_15
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
static void mdlOutputs(SimStruct *S, int_T tid) {
    ...
    int zeroPos = 492; //Potentiometer reading at level position
    int stepRes = 7328; //Number of steps to move the gimbal 180 degrees
    int potRes = 660; //Number of potValues that represent the rotation of the gimbal 180 degrees
    int potGive = 20; //Accounts for slack in gearing for pot
        ...
        case 0:{ 
            ...
            if (*potVal != 0) {
                ...
                if (counter == 0) {
                    ...
                    // Comment out remaining code in Option 0 as zeroing is all
                    // that is desired
...
//                 }
        ...
        // Commenting out remainder of code is not required as only
        // Option zero is given
