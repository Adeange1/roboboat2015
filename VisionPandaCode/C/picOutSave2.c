...
int i,test,failed,j,width,height,bytes_per_pixel, color_space,sockfd,len,result, jpglength;
...
static void mdlStart(SimStruct *S)
{
    ...
    const real_T  *P1  = mxGetData(PARAM_DEF0(S));
    const real_T  *P2  = mxGetData(PARAM_DEF1(S));
    const real_T  *P3  = mxGetData(PARAM_DEF2(S));
    const real_T  *P4  = mxGetData(PARAM_DEF3(S));
 failed=0;   
/*Determine Ip Address*/
 perror("pow1\n");fflush(stderr);
sprintf(temp,  "%d.%d.%d.%d%c",(int)*P1,(int)*P2,(int)*P3,(int)*P4,0);
ipaddr=(char*)temp;
port="8081";
...
perror("pow4\n");fflush(stderr);
	if(result == -1)
	{
        ...
        perror("Attempting to Start Webcam Server, Try again in a few sec\n");
