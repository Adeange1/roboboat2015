 ...
 *
  ...
  *  ------------------------------------------------------------------------- 
 * Created: Tue Aug  7 10:05:19 2012
 * 
 *
 */

#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME WebcamTest
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
//extern void WebcamTest_Outputs_wrapper(uint8_T *Red,
  //                        uint8_T *Green,
    //                      uint8_T *Blue);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    uint8_T        *Red  = (uint8_T *)ssGetOutputPortRealSignal(S,0);
    uint8_T        *Green  = (uint8_T *)ssGetOutputPortRealSignal(S,1);
    uint8_T        *Blue  = (uint8_T *)ssGetOutputPortRealSignal(S,2);
     ...
     //}
      ...
      unsigned char *data = frame->imageData;
      long i;
      long j = 0;
      long k = 0;
      for(i = 0; i < OUTPUT_0_WIDTH*OUTPUT_DIMS_0_COL*3; i+=3){
          //printf("data: %d, %d, %d \n",data[i],data[i+1], data[i+2]);
          Blue[j*OUTPUT_0_WIDTH+k] = data[i];
          Green[j*OUTPUT_0_WIDTH+k] = data[i+1];
          Red[j*OUTPUT_0_WIDTH+k] = data[i+2];
          j++;
          if(j==OUTPUT_DIMS_0_COL){k++; j = 0;}
      }
   //  char path[15];
     //strcpy(path,"test.jpg");
     //cvSaveImage(path, frame, 0);
//    WebcamTest_Outputs_wrapper(Red, Green, Blue);
}
