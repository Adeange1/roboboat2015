 ...
 *
  ...
  *  ------------------------------------------------------------------------- 
 * Created: Tue Jun 24 13:33:02 2014
 * 
 *
 */

#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME myReceiver6
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
// Variables to include in the global section of the S-Function
	int fd, i, n; // file descriptor for usb port and 
    char readBuffer[10]; 
    char placeHolder[1]; 
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    printf("Version8: RECEIVING..\n"); //Only changed size of Buffer to 1000
    ...
    for(i=0; i<10; i++)
        readBuffer[i]='K'; 
    printf("Cleared ReadBuffer3:%s\n",readBuffer); 
    tcflush(fd,TCIOFLUSH);  printf("FLUSHED\n"); 
