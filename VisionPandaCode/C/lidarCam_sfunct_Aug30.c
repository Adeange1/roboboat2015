...
#define S_FUNCTION_NAME lidarCam_sfunct_Aug30
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    int zeroPos = 445; //Potentiometer reading at level position
    ...
    }// End check to see if speed is positive
    
    else{
        long iEmpty;
        for(iEmpty=0;iEmpty<70200;iEmpty++){
             lidarData[iEmpty] = 0;
        }
    }
    // START CAM ACQ
