 ...
 *
  ...
  *  ------------------------------------------------------------------------- 
 * Created: Fri Jun 27 10:12:46 2014
 * 
 *
 */

#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME BayesExport
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
/* %%%-SFUNWIZ_defines_Changes_BEGIN --- EDIT HERE TO _END */
#define NUM_INPUTS          2
/* Input Port  0 */
#define IN_PORT_0_NAME      ColorProbs
...
extern void BayesExport_Outputs_wrapper(const real32_T *ColorProbs,
                          const int32_T *Bbox);
...
#define MDL_START  /* Change to #undef to remove function */
  ...
  {
    fpColorProbs = fopen("ColorProbData","w"); 
    if(fpColorProbs == NULL){ 
...
{
    const real32_T   *ColorProbs  = (const real32_T*) ssGetInputPortSignal(S,0);
    const int32_T   *Bbox  = (const int32_T*) ssGetInputPortSignal(S,1);
    
    for(i=0;i<3;i++) {
        fprintf(fpColorProbs,"%f \n", ColorProbs[i]);
    }
    for(j=0;j<4;j++) {
        fprintf(fpBbox,"%d \n", Bbox[j]);
    }

    //BayesExport_Outputs_wrapper(ColorProbs, Bbox);
}
