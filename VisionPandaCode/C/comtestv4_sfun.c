 ...
 *
  ...
  *  ------------------------------------------------------------------------- 
 * Created: Thu Jun 19 20:18:04 2014
 * 
 *
 */

#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME comtestv4_sfun
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
extern void comtestv2_sfun_Outputs_wrapper(const int32_T *color,
                          int32_T *run);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    const int32_T   *color  = (const int32_T*) ssGetInputPortSignal(S,0);
    int32_T        *run  = (int32_T *)ssGetOutputPortRealSignal(S,0);
    char strColor[1]; *run = 0; printf("HELLO");
    sprintf(strColor,"%d",*color); sendBuffer[12]=strColor[0]; 
	while(1){	 
    ...
    //comtestv2_sfun_Outputs_wrapper(color, run);
}
