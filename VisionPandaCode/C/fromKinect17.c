...
#define S_FUNCTION_NAME fromKinect17
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
unsigned char  kinR[480][640],kinG[480][640],kinB[480][640],blank[480][640];
freenect_context *kinCtx;
freenect_device *kinDev;
pthread_t kinThread;
unsigned int kinD[480][640];
double get_time()
{
	struct timeval cur;
...
void depth_cb(freenect_device *kinDev, void *data, uint32_t timestamp){ // Need to check size before doing bit wise opperations
    int i,j;
    unsigned char d[307200], depl, deph;
    //printf("depth=%d\n",FREENECT_DEPTH_11BIT_SIZE);
    for (i=0;i<FREENECT_DEPTH_11BIT_SIZE;i+=2)
    {
        j=i/2;
	depl = *((unsigned char*)data+i);
	deph = *((unsigned char*)data+i+1);
        kinD[j/640][j%640]= depl | deph<<8 ;
    }
    newFrame++;
...
void rgb_cb(freenect_device *kinDev, void *data, uint32_t timestamp)
{
    ...
    for (i=0;i<FREENECT_VIDEO_RGB_SIZE;i+=3)
    {
        j=i/3;
        kinR[j/640][j%640]=*((unsigned char*)data+i);
...
#define MDL_START  /* Change to #undef to remove function */
  ...
  static void mdlStart(SimStruct *S)
  {
    ...
    if(mode==0)
    {
        ...
        freenect_set_video_format(kinDev, FREENECT_VIDEO_RGB);
