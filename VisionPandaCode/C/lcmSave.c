 ...
 *
  ...
  *  ------------------------------------------------------------------------- 
 * Created: Tue Jul 22 17:40:06 2014
 * 
 *
 */

#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME lcmSave
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
FILE *lcmFD;
    char name[200]; 
extern void lcmSave_Outputs_wrapper(const real_T *pose,
                          const real_T *vel,
                          const real_T *euler,
...
#define MDL_START  /* Change to #undef to remove function */
  ...
  static void mdlStart(SimStruct *S)
  {
    ...
    sprintf(name,"lcmDataTime:%s",asctime(timeinfo)); 
    lcmFD = fopen(name,"w"); 
    if(lcmFD==NULL){
        printf("LCM File Not Made\n"); 
        exit(1); 
    }
  }
#endif /*  MDL_START */
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    fprintf(lcmFD,"pose: %g %g %g\n",pose[0],pose[1],pose[2]); 
    fprintf(lcmFD,"vel: %g %g\n",vel[0],vel[1]); 
    fprintf(lcmFD,"euler: %g %g %g\n",euler[0],euler[1],euler[2]); 
    fprintf(lcmFD,"portstate: %g %g\n",portstate[0],portstate[1]); 
    fprintf(lcmFD,"stbdstate: %g %g\n",stbdstate[0],stbdstate[1]); 
    fprintf(lcmFD,"Starttime: %g\n",*starttime); 
    fprintf(lcmFD,"ElapsedTime: %g\n",*elapsedtime); 
}
