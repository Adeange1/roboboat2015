 ...
 *
  ...
  *  ------------------------------------------------------------------------- 
 * Created: Tue Jul  1 21:45:30 2014
 * 
 *
 */

#define S_FUNCTION_LEVEL 2
#define S_FUNCTION_NAME lidar_webcam_test
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
/* Output Port  4 */
#define OUT_PORT_4_NAME      Blue
#define OUTPUT_4_WIDTH       480
#define OUTPUT_DIMS_4_COL    640
...
extern void lidar_webcam_test_Outputs_wrapper(const int32_T *potVal,
                          const real_T *Speed,
                          const real_T *Upos,
                          const real_T *Dpos,
...
#define MDL_START  /* Change to #undef to remove function */
  ...
  static void mdlStart(SimStruct *S)
  {
    ...
    //Webcam Setup
    capture  = cvCaptureFromCAM( -1 ); //Sets up Camera Object
    if ( !capture ) {
...
static void mdlOutputs(SimStruct *S, int_T tid) 
{
    ...
    uint8_T        *Red  = (uint8_T *)ssGetOutputPortRealSignal(S, 2);
    uint8_T        *Green  = (uint8_T *)ssGetOutputPortRealSignal(S, 3);
    uint8_T        *Blue  = (uint8_T *)ssGetOutputPortRealSignal(S, 4);
    ...
    // Otherwise scan
        ...
        IplImage *frame = cvQueryFrame( capture ); //Gets Frame from Camera
        //if ( !frame ) {
        //  fprintf( stderr, "ERROR: frame is null...\n" );
        //}
        
        unsigned char *vdata = frame->imageData;
        long i2;
        long j2 = 0;
        long k2 = 0;
        for(i2 = 0; i2 < OUTPUT_2_WIDTH*OUTPUT_DIMS_2_COL*3; i2+=3){
            //printf("data: %d, %d, %d \n",data[i],data[i+1], data[i+2]);
            Blue[j2*OUTPUT_2_WIDTH+k2] = vdata[i2];
            Green[j2*OUTPUT_2_WIDTH+k2] = vdata[i2+1];
            Red[j2*OUTPUT_2_WIDTH+k2] = vdata[i2+2];
            j2++;
            if(j==OUTPUT_DIMS_2_COL){k2++; j2 = 0;}
        }
    }
    
    //lidar_webcam_test_Outputs_wrapper(potVal, Speed, Upos, Dpos, ScanNumber, lidarData, udOut, Red, Green, Blue);
}
