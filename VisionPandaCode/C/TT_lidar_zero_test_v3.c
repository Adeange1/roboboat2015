...
#define S_FUNCTION_NAME TT_lidar_zero_test_v3
/*<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    int zeroPos = 517;
    int stepRes = 1036; //Number of steps to move the stepper 45 degrees
    ...
    int dstep;
                ...
                CPhidgetStepper_setTargetPosition(stepper, 0, (4144+));
    while(1); 
    /*
