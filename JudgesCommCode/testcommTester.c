...
// 		Last date modified: 7/14/2014, 6:15PM
//**************************************************************************************
...
int main(){
	...
	// start a run
	printf("Starting run:\n");
	str = server_requestPOSTLight("http://ec2-52-7-253-202.compute-1.amazonaws.com/run/start/courseB/VU");
	if(str == NULL){ 
	...
	str = server_requestPOST("http://ec2-52-7-253-202.compute-1.amazonaws.com/run/start/courseB/VU", acousticStr);
	if(str == NULL){ 
		printf("Request Not Executed\n");
	} else{ 
		printf("main:%s\n\n",str);
...
*/
	...
	str = server_requestGET("http://ec2-52-7-253-202.compute-1.amazonaws.com/automatedDocking/courseB/VU");
	if(str == NULL){ 
		printf("Request Not Executed\n");
	} else{ 
		printf("main:%s\n\n",str);
	...
	str = server_requestGET("http://ec2-52-7-253-202.compute-1.amazonaws.com/obstacleAvoidance/courseB/VU"); 
	if(str == NULL){ 
		printf("Request Not Executed\n");
	} else{ 
		printf("main:%s\n\n",str);
...
*/
	...
	char *acousticStr = acousticObject(4, 36.80281, -76.19152); 
	str = server_requestPOST("http://ec2-52-7-253-202.compute-1.amazonaws.com/pinger/courseB/VU", acousticStr);
	if(str == NULL){ 
		printf("Request Not Executed\n");
	} else{ 
		printf("main:%s\n\n",str);
	...
	//ending run
	printf("Ending run:\n");
	str = server_requestPOSTLight("http://ec2-52-7-253-202.compute-1.amazonaws.com/run/end/courseB/VU");
	if(str == NULL){ 
		printf("Request Not Executed\n");
	} else{ 
		printf("main:%s\n\n",str);
